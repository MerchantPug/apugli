...
public class ModifyEquippedItemRenderPower extends Power {
    ...
    public static PowerFactory<?> getFactory() {
        return new PowerFactory<ModifyEquippedItemRenderPower>(Apugli.identifier("modify_equipped_item_render"),
                ...
                data ->
                        (type, player) ->
                                new ModifyEquippedItemRenderPower(type, player, (EquipmentSlot)data.get("slot"), (ItemStack)data.get("stack"), data.getFloat("scale"), data.getBoolean("override"), data.getBoolean("merge_with_held")))
                .allowCondition();
    }
