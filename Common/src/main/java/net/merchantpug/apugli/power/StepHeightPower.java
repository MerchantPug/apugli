...
public class StepHeightPower extends Power {
    ...
    public StepHeightPower(PowerType<?> powerType, LivingEntity entity, float lowerCorrectionRange, float upperCorrectionRange) {
        super(powerType, entity);
        this.lowerCorrectionRange = lowerCorrectionRange;
        this.upperCorrectionRange = upperCorrectionRange;
    }

    private boolean canCorrectLowerHeight() {
        return lowerCorrectionRange > 0.0;
    }

    private boolean canCorrectUpperHeight() {
        return upperCorrectionRange > 0.0;
    }

    public float getLowerCorrectionRange() {
        return lowerCorrectionRange;
    }

    public float getUpperCorrectionRange() {
        return upperCorrectionRange;
    }

    public static Optional<Float> getLowerCorrectionRange(LivingEntity living) {
        return Services.POWER.getPowers(living, ApugliPowers.STEP_ASSIST.get())
                .stream()
