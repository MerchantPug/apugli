...
public interface ModifyEnchantmentLevelPowerFactory<P> extends ValueModifyingPowerFactory<P> {
    ...
    default ListTag generateEnchantments(ListTag enchants, ItemStack self) {
        ...
        for (P power : powers) {
            ...
            if(idx.isPresent()) {
                ...
                int newLvl = (int) Services.PLATFORM.applyModifiers(this.getModifiers(power, living), lvl);
                ...
                newEnchant.putInt("lvl", (int) Services.PLATFORM.applyModifiers(this.getModifiers(power, living), 0));
    ...
    default ListTag getEnchantments(ItemStack self, ListTag originalTag) {
        ...
        if (entity instanceof LivingEntity living && getEntityItemEnchants().containsKey(entity.getStringUUID())) {
            ...
            if (shouldReapplyEnchantments(living, self, originalTag)) {
                ...
                return itemEnchants.compute(originalTag, (original, tag) -> generateEnchantments(original, self));
            }
    ...
    default boolean shouldReapplyEnchantments(LivingEntity living, ItemStack self, ListTag originalTag) {
        ...
        ConcurrentHashMap<P, Tuple<Integer, Boolean>> cache = getPowerModifierCache().computeIfAbsent(living.getStringUUID(), (_uuid) -> new ConcurrentHashMap<>());
        return !enchants.containsKey(originalTag) || powers.stream().anyMatch(power -> updateIfDifferent(cache, power, (int) Services.PLATFORM.applyModifiers(getModifiers(power, living), 0), Services.POWER.isActive(power, living) && checkItemCondition(power, living.getLevel(), self)));
    }
